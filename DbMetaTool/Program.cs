...
public static class Program
{
    ...
    private static void ExportDomains(FbConnection connection, string outputDirectory)
    {
        ...
        using (var r = cmd.ExecuteReader())
        {
            while (r.Read())
            {
                var name = r["DOMAIN_NAME"] as string;
                if (name is null)
                ...
                var fieldType = Convert.ToInt32(r["RDB$FIELD_TYPE"]);
                var length = Convert.ToInt32(r["RDB$FIELD_LENGTH"]);
                var charLength = r["RDB$CHARACTER_LENGTH"] is DBNull
                    ? -1
                    : Convert.ToInt32(r["RDB$CHARACTER_LENGTH"]);
                var scale = Convert.ToInt32(r["RDB$FIELD_SCALE"]);
                var charsetId = r["RDB$CHARACTER_SET_ID"] is DBNull
                    ? -1
                    : Convert.ToInt32(r["RDB$CHARACTER_SET_ID"]);

                var defaultSrc = r["RDB$DEFAULT_SOURCE"] as string;
                var checkSrc = r["RDB$VALIDATION_SOURCE"] as string;
                var notNull = r["RDB$NULL_FLAG"] != DBNull.Value;

                var dataType = GetDataType(fieldType, length, charLength, scale);
    ...
    private static string ExportTableDefinition(FbConnection connection, string table)
    {
        ...
        while (r.Read())
        {
            ...
            var field = r["FIELD_NAME"].ToString();
            var domainName = r["DOMAIN_NAME"].ToString();
            ...
            var fieldType = Convert.ToInt32(r["RDB$FIELD_TYPE"]);
            var length = Convert.ToInt32(r["RDB$FIELD_LENGTH"]);
            var charLength = r["RDB$CHARACTER_LENGTH"] is DBNull ? -1 : Convert.ToInt32(r["RDB$CHARACTER_LENGTH"]);
            var scale = Convert.ToInt32(r["RDB$FIELD_SCALE"]);
            var charsetId = r["RDB$CHARACTER_SET_ID"] is DBNull ? -1 : Convert.ToInt32(r["RDB$CHARACTER_SET_ID"]);

            var defaultSrc = r["RDB$DEFAULT_SOURCE"] as string;
            var checkSrc = r["RDB$VALIDATION_SOURCE"] as string;
            var notNull = r["RDB$NULL_FLAG"] != DBNull.Value;

            var dataType = !string.IsNullOrWhiteSpace(domainName)
                ? domainName
    ...
    private static void ExportProcedures(FbConnection connection, string outputDirectory)
    {
        ...
        while (r.Read())
        {
            var name = (r["NAME"] as string)?.Trim() ?? throw new Exception("Nazwa procedury jest null.");
    ...
    private static string BuildProcedureDefinition(FbConnection connection, string procedureName)
    {
        ...
        using (var cmd = new FbCommand(sqlInputParams, connection))
        {
            ...
            while (r.Read())
            {
                ...
                var paramName = r["PARAM_NAME"].ToString()!;
                var domainName = r["DOMAIN_NAME"].ToString()!;
        ...
        using (var cmd = new FbCommand(sqlProc, connection))
        {
            ...
            if (r.Read())
            {
                var source = r["RDB$PROCEDURE_SOURCE"] as string ?? string.Empty;
                sb.AppendLine("AS");
    ...
    private static (int Executed, List<string> Failed) RunFolderSqlFiles(FbConnection connection, string folderPath,
        Action<FbConnection, string, FbTransaction?> executor, FbTransaction? transaction = null)
    {
        var failed = new List<string>();
        var executed = 0;
